...
from tools import call_openai_api, SYSTEM_PROMPT, build_chat_conversation, update_chat_conversation, logger, escape_markdown_v2, HR_PROMPT
...
async def chat_with_bot_handler(query: types.CallbackQuery, state: FSMContext):
    ...
    question_btn = InlineKeyboardButton(text='Маю запитання', callback_data='question_btn')
...
async def improve_cv_handler(query: types.CallbackQuery, state: FSMContext):
    ...
    # Append the AI answer to the conversation list
    ...
    # Update the state with the conversation (merging with any existing data)
...
async def handle_cv_improvement(message: types.Message, state: FSMContext):
    ...
    # Optionally update conversation context
    updated_conversation = await update_chat_conversation(conversation, user_query=message.text)
    ai_answer = await call_openai_api(updated_conversation)
    ...
    if not insertion_id:
        ...
        cvs_db.update_chat_conversation(insertion_id, conversation)
...
async def question_btn_handler(query: types.CallbackQuery, state: FSMContext):  
    ...
    hr_conversation.append({"role": "system", "content": HR_PROMPT})
...
async def handle_question(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    conversation = data.get("hr_conversation", [])
    conversation.append({"role": "user", "content": message.text})
    ...
    conversation.append({"role": "assistant", "content": ai_answer})
    await state.update_data(hr_conversation=conversation)
